...
echo '<form method = "post" action = "http://localhost:8080/new_batch_marketing/reports/receivables_redirect.php">';
...
$infos = $new_batch_marketing->query("SELECT p.transaction_id,a.lastname, a.firstname, a.contact_num, c.customer_account_id, c.customer_firstName, c.customer_lastName, c.barangay, c.town, c.customer_contact_number, cd.customer_delivery_id, cd.date, cd.delivered_by, co.order_date, co.delivery_date, co.purchase_order_id, co.customer_id,co.amount
...
WHERE c.customer_account_id =  '$cust_id'");

  foreach($infos as $key){
    $original_amount = $key['amount'];
  ...
  <div style = "margin-left: 40%;">NEW BATCH MARKETING</div>
      ...
      <tr><td>TRANSACTION ID:</td><td>'.$transaction_id.'</td><td colspan = "3">Processed by:</td><td>'.$processed_by.'</td></tr>
    </table></div>';
  ...
  $get_acoount_details = $new_batch_marketing->query("SELECT pd.OR,pd.date,pd.amount_paid,p.amount_payable,co.delivery_date FROM customer_order AS co
    ...
    WHERE co.purchase_order_id = '$order_id'");

  if(empty($get_acoount_details)){
...
while($aging >0){
  ...
  foreach($get_acoount_details as $key){
    if($key['date'] == $temp_date){
      ...
      $new_balance = round($new_balance - $a_paid,2,PHP_ROUND_HALF_UP);
      if($new_balance < 1){
        $new_balance = 0;
      }
      $date_flag = $key['date'];
 ...
 // if(isset($_POST['ok'])){
 //    echo '<script type="text/javascript">window.location.href = "receivables.php";</script>';
 //    exit();
 // }
 echo '</form>';
 echo' <footer style = "margin-top:37%;"></footer>';
echo'</div>';
?>
